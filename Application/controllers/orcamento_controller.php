...
if (isset($_POST['editar_orcamento'])) {
    $id_orcamento = mysqli_real_escape_string($conexao, $_POST['id_orcamento']);
    
    // Excluir o orçamento e todas as suas dependências
    OrcamentoDAO::excluirOrcamento($id_orcamento);
    
    // Agora recriar o orçamento com as novas informações fornecidas
    $nome_orcamento = mysqli_real_escape_string($conexao, $_POST['nome_orcamento']);
    $data_orcamento = mysqli_real_escape_string($conexao, $_POST['data_orcamento']);
    $validade = mysqli_real_escape_string($conexao, $_POST['validade']);
    $status = mysqli_real_escape_string($conexao, $_POST['status']);
    ...
    $novoIdOrcamento = OrcamentoDAO::criarOrcamento($nome_orcamento, $data_orcamento, $validade, $status, $observacao, $caminho_arquivo, $fk_cliente_id, $valor_total_orcamento);

    if ($novoIdOrcamento > 0) {
        // Decodificar dados dos materiais e serviços, se houverem
        $materiais = isset($_POST['materiaisCapturados']) ? json_decode($_POST['materiaisCapturados'], true) : [];
        $servicos = isset($_POST['servicosCapturados']) ? json_decode($_POST['servicosCapturados'], true) : [];
        ...
        foreach ($_POST['nome_item'] as $index => $nomeItem) {
            ...
            // Criar o item de orçamento
            $itemId = ItensOrcamentoDAO::criarItemOrcamento($novoIdOrcamento, $nome_item, $descricaoItem, $valor_total_item);
